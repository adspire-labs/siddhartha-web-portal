...
export default function Gallery() {
  ...
  const getAllGalleryDataApi = async () => {
    try {
      ...
      const res = await axios.get(`${apiEndpoint.fetchGallery}?t=${Date.now()}`);
      ...
      {lightboxOpen && (
        <div className="fixed inset-0 bg-black/90 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative max-w-4xl w-full">
            <img src={currentGroup[currentImageIndex].photo} alt="Gallery" className="max-h-[80vh] w-full object-contain" />
            <div className="absolute inset-x-0 bottom-4 flex justify-between px-4">
              <button onClick={() => navigateLightbox('prev')} className="bg-white/20 hover:bg-white/30 rounded-full p-2">
                <ChevronLeft className="w-6 h-6 text-white" />
              </button>
              <div className="text-white text-center">
                {currentImageIndex + 1} / {currentGroup.length}
              </div>
              <button onClick={() => navigateLightbox('next')} className="bg-white/20 hover:bg-white/30 rounded-full p-2">
          ...
          {galleryData.length === 0 ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {Object.entries(groupedGallery).map(([title, items]) => (
                <Card key={title} className="overflow-hidden shadow-card hover:shadow-elegant transition-smooth cursor-pointer group" onClick={() => openLightbox(title)}>
                  <div className="relative aspect-square overflow-hidden">
                    <img src={items[0].photo} alt={title} className="w-full h-full object-cover group-hover:scale-105 transition-smooth" />
                    {items.length > 1 && (
