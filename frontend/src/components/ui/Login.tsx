...
export default function AdminLogin() {
  ...
  // Store default credentials only once
  useEffect(() => {
    const defaultCredentials = {
      email: "sebss@gmail.com",
      password: "Optimistic@2082",
    };
    if (!localStorage.getItem("defaultAdminCredentials")) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!defaultCreds) {
      toast({
        title: "Error",
        description: "Default credentials missing. Please try again later.",
        variant: "destructive",
      });
      return;
    }
    const { email: defaultEmail, password: defaultPass } =
      JSON.parse(defaultCreds);
    const userCredentials = { email, password };

    if (email === defaultEmail && password === defaultPass) {
      localStorage.setItem("adminCredentials", JSON.stringify(userCredentials));
